...
SELECT 
    ...
    ROUND(ST_Distance(vp.current_location, sos.location) / 1000, 2) as distance_km
FROM public.volunteer_profiles vp
JOIN public.users u ON vp.user_id = u.id
CROSS JOIN (
    SELECT location FROM public.sos_requests WHERE id = 'sos-uuid-here'
) sos
...
FROM public.hubs h
CROSS JOIN (
    SELECT location FROM public.aid_requests WHERE id = 'aid-request-uuid'
) ar
...
SELECT 
    ...
    ROUND(ST_Distance(sos.location, vp.current_location) / 1000, 2) as distance_km,
    sos.created_at
...
ORDER BY 
    -- Priority: CRITICAL > HIGH > MEDIUM > LOW, then by distance
    CASE sos.urgency_level 
...
ORDER BY m.priority_score DESC, m.created_at ASC;
...
DECLARE
    v_urgency_weight NUMERIC;
    v_distance_score NUMERIC;
    v_time_score NUMERIC;
    v_people_score NUMERIC;
BEGIN
    -- Urgency weight (CRITICAL = 100, HIGH = 70, MEDIUM = 40, LOW = 10)
